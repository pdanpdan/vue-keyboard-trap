...
function createCtx(config, el, value, modifiers) {
  const ctx = {
    disable: value === false,
    modifiers,
...
function bindFn(config, el, value, modifiers) {
  const ctx = createCtx(config, el, value, modifiers);
...
function updateFn(config, ctx, el, value, modifiers) {
...
export default function directiveFactory(options) {
  ...
  const unmounted = (el) => unbindFn(config, el);
...
// helper because vue-demi does not have it
function toValue(maybeRef) {
  return typeof maybeRef === 'function'
    ? maybeRef()
    : unref(maybeRef);
}

export function composableFactory(options) {
  const config = createConfig(options);

  return (maybeElementOrComponentRefOrComputed, modifiersRefOrComputed = {}, activeRefOrComputed = true) => {
    const elComputed = computed(() => {
      const elOrComponent = toValue(maybeElementOrComponentRefOrComputed);
      if (elOrComponent == null) {
        return null;
      }
      return markRaw('$el' in elOrComponent ? elOrComponent.$el : elOrComponent);
    });

    const unwatch = watch(() => [elComputed.value, toValue(activeRefOrComputed), toValue(modifiersRefOrComputed)], ([el, value, modifiers], [oldEl] = []) => {
      if (el == null && oldEl == null) {
        return;
      }

      if (oldEl == null && el != null) {
        bindFn(config, el, value, modifiers);
      } else if (el == null) {
        unbindFn(config, el);
      } else if (el !== oldEl) {
        unbindFn(config, oldEl);
        bindFn(config, el, value, modifiers);
      } else {
        updateFn(config, getCtx(el), el, value, modifiers);
      }
    ...
    if (getCurrentScope()) {
      onScopeDispose(() => {
        unwatch();
        if (elComputed.value != null) {
          unbindFn(config, elComputed.value);
        }
      });
    }
  };
