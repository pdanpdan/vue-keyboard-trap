f<template>
  ...
  <div>
    ...
    <button @click="toggleTrapRoving">{{ trapModifiers.roving === true ? 'Arrow navigation' : 'Tab navigation' }}</button>

    <div ref="elRef" class="test">
...
import { useKeyboardTrapFactory } from '@pdanpdan/vue-keyboard-trap'; // asGlobal="VueKeyboardTrap"
...
export default {
  setup() {
    ...
    const trapModifiers = ref({ autofocus: true, roving: false });
    const toggleTrapActive = () => {
      trapActive.value = trapActive.value !== true;
    };
    const toggleTrapRoving = () => {
      trapModifiers.value.roving = trapModifiers.value.roving !== true;
    };

    useKeyboardTrap(elRef, trapModifiers, trapActive);
